...
//prepare statements
$stmtAccessibilityToilets = $conn->prepare("SELECT * FROM accessibilitytoilets WHERE accessibilitytoilets.id = $id");
//executes
$stmtAccessibilityToilets->execute();
//close connection
$conn = null;
//fetch data
$accessibilityToiletsPHP = $stmtAccessibilityToilets->fetch();
...
<head>
    ...
    <title>Accessibility Toilet Edit</title>
</head>
...
<body class="admin-body">
    <div class="container-fluid">
        <nav class="row">
            ...
            <div class="col-sm-12 col-md-12 col-lg-4 col-xl-4 text-center admin-title ">Edit Accessibility Toilet</div>
    ...
    <main class="main">
        ...
        <form id="neutralForm" action="accessibilityUpdate.php">
...
<script>
    (function() {
        function initialise() {
            ...
            const map = createMap();
            if (id === null) {
                location.replace("accessibilityToilets.php?iderror=true");
            }
            ...
            function createAccessibilityToilet(map, accessibilityToilets) {
                //create a marker, grab the coordinates from the list of group markers, stick it on the map and apply the custom icon
                const coords = new google.maps.LatLng(accessibilityToilets.lat, accessibilityToilets.lng);
                if (accessibilityToilets.id == id) {
                    formInit(accessibilityToilets);
                }
                var accessibilityToilet = new google.maps.Marker({
                    position: new google.maps.LatLng(parseFloat(accessibilityToilets.lat), parseFloat(accessibilityToilets.lng)),
                    map: map,
                    icon: iconBase + 'disabledtoilet' + png,
                    draggable: true
                });
                markerCoords(accessibilityToilet, map);

                map.setCenter(coords);
                accessibilityToiletHTMLSetup(accessibilityToilets, accessibilityToilet);
            }

            function accessibilityToiletHTMLSetup(accessibilityToilets, accessibilityToilet) {
                ...
                markerLat.setAttribute("value", accessibilityToilets.lat);
                markerLng.setAttribute("value", accessibilityToilets.lng);
                buildingName.setAttribute("value", accessibilityToilets.building_name);
                //reset button to move marker to original position if moved
                $("#resetLatLng").click(function() {
                    var latlng = new google.maps.LatLng(accessibilityToilets.lat, accessibilityToilets.lng);
                    accessibilityToilet.setPosition(latlng);
                    markerLat.setAttribute("value", accessibilityToilets.lat);
                    markerLng.setAttribute("value", accessibilityToilets.lng);
                });
                const roomCodes = accessibilityToilets.room_name.split(', ');
            ...
            function formInit(accessibilityToilets) {
                $("#buildingName").attr("value", accessibilityToilets.building_name);
            }


            function main(map, accessibilityToilets) {
                createAccessibilityToilet(map, accessibilityToilets)
            }
