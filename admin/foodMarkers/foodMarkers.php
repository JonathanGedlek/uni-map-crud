...
<body class="admin-body">
    <div class="container-fluid">
        <nav class="row">
            ...
            <div class="col-sm-12 col-md-12 col-lg-4 col-xl-4 text-center admin-title ">Food Markers</div>
    ...
    <button class="btn btn-info" type="button"><a href="foodCreate.php">Create New Marker</a></button>
    <div id="error">
    </div>
</body>
...
<script>
    (function() {
        function initialise() {
            ...
            function createFoodMarker(map, foodMarkers, i) {
            ...
            function createFoodInfo(foodMarker, foodMarkers, map, i) {
                var foodMarkerInfo = new google.maps.InfoWindow();
                var foodText = '<div class="text"><h4>' + foodMarkers[i].location + ' Food</h4><br><a href="foodEdit.php?id=' + foodMarkers[i].id + '">Edit</a><br><br><a onclick="return confirm(`Are you sure you want to delete this marker?`)" id="deleteBtn'+i+'"href="foodDelete.php?id=' + foodMarkers[i].id + '">Delete</a>';
                foodText += "</div>";
                foodMarkerInfo.setContent(foodText);
                foodMarkerInfo.setMap(null);
                google.maps.event.addListener(foodMarker, 'click', function() {
                    if (foodMarkerInfo.getMap() === null) {
                        foodMarkerInfo.open(map, foodMarker);
                    } else {
                        foodMarkerInfo.close();
                    }
                });
            }
            ...
            function initialiseFoodMarkers(map, foodMarkers) {
                for (i = 0; i < foodMarkers.length; i++) {
                    var foodMarker = createFoodMarker(map, foodMarkers, i);
